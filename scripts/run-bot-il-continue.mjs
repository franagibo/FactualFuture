...
function main() {
  ...
  // 1) Continue training from existing weights (if present). Use full path so output is in bot-training/.
  runStep(
    pythonCmd,
    [
  ...
  // 2) Export weights to JSON for the engine. Use full path so output is in bot-training/.
  runStep(
    pythonCmd,
    [
